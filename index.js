...
function findElementGreaterThan(arr, num) {
  ...
  let foundElem = arr.find((elem) => elem > num);
...
function getPositiveArrayValues(arr) {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
  if (!Array.isArray(arr)) return "Вхідний параметр має бути масивом";
  let iterator = Array.from(arr.values());
  ...
  return iterator.filter((elem) => elem > 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  let powArr = newArr.map((elem) => Math.pow(elem, 2));
  // За допомогою методу filter() фільтруємо елементи, які більше за порогове значення.
  return powArr.filter((elem) => elem > threshold);
...
function wordLengths(arr) {
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
  if (!Array.isArray(arr)) return "Вхідний параметр має бути масивом";
  return arr.map((elem) => Array.from(elem).length);
...
function expandArrayByN(arr, n) {
  ...
  let result = arr.flatMap((element) => {
    let newArr = [];
...
function findLongestWord(arr) {
  ...
  return arr.reduce((accumulator, currentValue) => {
    if (accumulator.length > currentValue.length) return accumulator;
...
function findDuplicateElements(arr) {
  ...
  let dublicateElements = arr.filter((num) => {
...
function capitalizeWords(arr) {
  ...
  return arr
    .map((word) => {
      let wordFromArray = Array.from(word);
      wordFromArray.splice(0, 1, wordFromArray[0].toUpperCase());
      return wordFromArray.join("");
    })
    .flat();
  // Перетворюємо першу літеру слова у верхній регістр та додамо до неї всі символи слова крім першого
  //Повертаємо слово
